...
with gr.Blocks() as app:
    ...
    with gr.Row():
        ...
        with gr.Column():
            lotarea_record = gr.Number(label="Lot Area", value=10000)
            overallqual_record = gr.Number(label="Overall Quality", value=7)
            overallcond_record = gr.Number(label="Overall Condition", value=5)
            centralair_record = gr.Radio(["Yes", "No"], label="Central Air", value="Yes")
            fullbath_record = gr.Number(label="Full Bathrooms", value=2)
            bedroomabvgr_record = gr.Number(label="Bedrooms Above Grade", value=3)
            garagecars_record = gr.Number(label="Garage Cars", value=2)
            saleprice_record = gr.Number(label="Verified Sale Price", value=250000)
